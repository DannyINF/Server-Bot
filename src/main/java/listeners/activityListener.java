...
public class activityListener extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        String[] answer = null;
        try {
            answer = core.databaseHandler.database(STATIC.getGuild().getId(), "select recent_activity from users where id = '" + event.getAuthor().getId() + "'");
        ...
        if (answer == null || answer.length == 0) {
            try {
                core.databaseHandler.database(STATIC.getGuild().getId(), "insert into users (id, intro, profile, words, msg, chars, voicetime, verifystatus, activity, recent_activity, first_join, last_join, language, country, sex, xp, level) " +
        ...
        try {
            core.databaseHandler.database(STATIC.getGuild().getId(), "update users set activity = activity + " + activity + ", " +
        ...
        if (!Objects.requireNonNull(event.getMember()).getRoles().contains(STATIC.getNewcomer()[0])) {
            ...
            try {
                answerRoles = core.databaseHandler.database(STATIC.getGuild().getId(), "select {fn timestampdiff(SQL_TSI_FRAC_SECOND, last_join, CURRENT_DATE)} / 86400000000000 from users where id = '" + event.getAuthor().getId() + "'");
            } catch (SQLException e) {
                e.printStackTrace();
            }
