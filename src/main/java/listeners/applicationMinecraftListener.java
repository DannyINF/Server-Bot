...
public class applicationMinecraftListener extends ListenerAdapter {
    public void onPrivateMessageReceived(@NotNull PrivateMessageReceivedEvent event) {
        String[] answer = null;
        try {
            answer = core.databaseHandler.database(STATIC.getGuild().getId(), "select * from applications where id = '" + event.getAuthor().getId() + "' and step < 8");
        ...
        if (answer.length != 0 && answer[1].length() == 1) {
            switch (answer[1]) {
                case "1":
                    ...
                    try {
                        core.databaseHandler.database(STATIC.getGuild().getId(), "update applications set step = 2, name = '" + name + "' where id = '" + event.getAuthor().getId() + "' and step = 1");
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                    break;
                case "2":
                    ...
                    try {
                        core.databaseHandler.database(STATIC.getGuild().getId(), "update applications set step = 3, age = '" + age + "' where id = '" + event.getAuthor().getId() + "' and step = 2");
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                    break;
                case "3":
                    ...
                    try {
                        core.databaseHandler.database(STATIC.getGuild().getId(), "update applications set step = 4, location = '" + event.getMessage().getContentRaw().replace("'", "\"") + "' where id = '" + event.getAuthor().getId() + "' and step = 3");
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                    break;
                case "4":
                    ...
                    try {
                        core.databaseHandler.database(STATIC.getGuild().getId(), "update applications set step = 5, language = '" + event.getMessage().getContentRaw().replace("'", "\"") + "' where id = '" + event.getAuthor().getId() + "' and step = 4");
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                    break;
                case "5":
                    ...
                    event.getChannel().sendMessage(">>> Warum solltest gerade DU Minecraft-GameMaster werden? Erz\u00e4hle uns, was du f\u00FCr Ideen und Erfahrungen in diesem Bereich hast! (max. 1.000 Zeichen)").queue();
                    try {
                        core.databaseHandler.database(STATIC.getGuild().getId(), "update applications set step = 6, about_you = '" + event.getMessage().getContentRaw().replace("'", "\"") + "' where id = '" + event.getAuthor().getId() + "' and step = 5");
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                    break;
                case "6":
                    ...
                    try {
                        core.databaseHandler.database(STATIC.getGuild().getId(), "update applications set step = 7, change = '" + event.getMessage().getContentRaw().replace("'", "\"") + "' where id = '" + event.getAuthor().getId() + "' and step = 6");
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
                    break;
    ...
    public void onPrivateMessageReactionAdd(PrivateMessageReactionAddEvent event) {
        String[] answer = null;
        try {
            answer = core.databaseHandler.database(STATIC.getGuild().getId(), "select * from applications where id = '" + event.getUserId() + "' and step = 7");
        } catch (SQLException e) {
            e.printStackTrace();
        }
        assert answer != null;
        if (answer.length != 0 && answer[1].length() == 1) {
            ...
            embed.setDescription("Der Nutzer **" + Objects.requireNonNull(STATIC.getGuild().getMemberById(answer[0])).getUser().getAsTag() + "** hat sich als **Minecraft-GameMaster** beworben.");
            ...
            STATIC.getModlog().getGuild().getTextChannelById("831194532502568981"). sendMessageEmbeds(embed.build()).queue(msg -> {
                try {
                    core.databaseHandler.database(STATIC.getGuild().getId(), "delete from applications where id = '" + event.getUserId() + "' and step = 7");
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            });
            STATIC.getApply().sendMessage(">>> Der Nutzer **" + Objects.requireNonNull(STATIC.getGuild().getMemberById(answer[0])).getUser().getAsTag() + "** hat sich als **Minecraft-GameMaster** beworben. Gib `/bewerben` oder `/apply` ein, wenn du dich auch als Moderator bewerben m\u00f6chtest.").queue();
