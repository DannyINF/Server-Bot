...
public class cmdMinecraftApply implements Command {
    @Override
    ...
    public void action(String[] args, GuildMessageReceivedEvent event) throws SQLException {
        ...
        core.databaseHandler.database(event.getGuild().getId(), "delete from applications where id = '" + event.getAuthor().getId() + "' and step < 10");
        ...
        core.databaseHandler.database(event.getGuild().getId(), "insert into applications (id, step, name, age, location, language, about_you, change) " +
